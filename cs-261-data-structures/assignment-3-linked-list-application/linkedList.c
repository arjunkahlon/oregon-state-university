...
* Date Created: February 3rd, 2019
...
void linkedListPrint(struct LinkedList* deque)
{
    ...
    struct Link* printLink = deque->frontSentinel->next;

    for (int i = 0; i < deque->size; i++)
    {
        printf(FORMAT_SPECIFIER, printLink->value);
        printf(" ");
...
int linkedListContains(struct LinkedList* bag, TYPE value)
{
    ...
    /* FIXME: You will write this function */
    assert(bag != NULL);
    struct Link* compareLink = bag->frontSentinel->next;

    for (int i = 0; i < bag->size; i++)
    {
        if (compareLink->value == value)
            return 1;
        compareLink = compareLink->next;
    }
...
void linkedListRemove(struct LinkedList* bag, TYPE value)
{
    /* FIXME: You will write this function */
    assert(bag != NULL);
    struct Link* linkToRemove = bag->frontSentinel->next;

    for (int i = 0; i < bag->size; i++)
    {
        if (linkToRemove->value == value)
        {
            removeLink(bag, linkToRemove);
            return;
        }
