/***********************************************************
* Author: Arjun Kahlon
* Email: kahlona@oregonstate.edu
* Date Created: February 15th, 2019
* Filename: bst.c
*
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    int cResult = compare(cur->val, val);
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    /*write this*/
    assert(tree != 0);
    //assert (val != 0);    ask professor
    struct Node *currentNode = tree->root;
    while (currentNode != NULL)
    {
        int result = compare(val, currentNode->val);
        if(result == 0)
            return 1;
        if(result < 0)
        {
...
struct Node *_removeLeftMost(struct Node *cur)
{
    ...
    //check to see if left child is not NULL. If left child is not NULL, we recurse through the function until we
    //reach left most.
    if (cur->left != NULL)
    {
        cur->left = _removeLeftMost(cur->left);  //recursive call
        return cur;
    }

    //return right child of curr if curr is NULL
    struct Node *rightChildNode = cur->right;
    free(cur);       //frees cur
...
int main(int argc, char *argv[]){
 ...
 points */
    ...
    /*struct data *test1 = (struct data*)malloc(sizeof(struct data));
    test1->number = 12;

    printf("%d",compare(test1, test1));*/
